...
class AlpacaCollector:
    ...
    def get_latest_quote(self, symbol):
        """Fetch the latest quote (Bid/Ask) for spread check"""
        try:
            req = StockQuotesRequest(symbol_or_symbols=symbol, limit=1)
            # keys are symbols, values are lists of quotes
            res = self.data_client.get_stock_quotes(req)
            if symbol in res:
                return res[symbol][0]
        except Exception as e:
